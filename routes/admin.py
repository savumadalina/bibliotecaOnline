...
def loans():
	...
	id = int(admin_manager.admin.uid())
	admin = admin_manager.get(id)
	books = book_manager.listLoans()

	return render_template('books/loans.html', g=g, books=books, admin=admin)

@admin_view.route('/books/loans/<int:id_reservation>', methods=['GET'])
@admin_manager.admin.login_required
def loansReservation(id_reservation):
	admin_manager.admin.set_session(session, g)
	id = int(admin_manager.admin.uid())
	admin = admin_manager.get(id)
	book_manager.loan(id_reservation)
	books = book_manager.listLoans()

	return redirect('/admin/books/loans')
...
def view_book(id):
	...
	if id != None:
		...
		return render_template("books/book_view.html", books=b, g=g)
...
def book_add():
	...
	if "title" in request.args:
		...
		pg = request.args["pg"]
		language = request.args["limba"]
		image = request.args["imagine"]

		book_manager.addBook(title, author, genre, pg, language, image)
...
def search():
	...
	d=book_manager.searchEmail(keyword, 0)

	if len(d) >0:
		return render_template("books/loans.html", search=True, books=d, count=len(d), keyword=escape(keyword), g=g, admin=admin)

	return render_template('books/loans.html', error="No books found!", keyword=escape(keyword))
